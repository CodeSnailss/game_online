 ...
 */
    ...
    for (var i = 0; i < scpts.length; i++) {
        ...
        div.style.left = Math.round(xx + (ww - 700) / 2) + 'px';
...
//var flap = loadGameSound(cndSrc + 'flap.wav', 3);

//var coin = loadGameSound(cndSrc + 'coin.wav', 1);

//var punch = loadGameSound(cndSrc + 'punch.wav', 1);
...
wing.img = loadGameImage(cndSrc + 'wing.png'); // 150 x 131
...
wingfill.img = loadGameImage(cndSrc + 'wingfill.png'); // 150 x 131
...
logo.showing = true;
logo.hiding = false;
logo.img = loadGameImage(cndSrc + 'logo.png'); // 626 x 188
...
gameover.showing = false;
gameover.hiding = true;
gameover.img = loadGameImage(cndSrc + 'gameover.png'); // 626 x 144
...
fbshare.img.onclick = function(e) {
    /*FB.ui({
        method: 'feed',
...
c2s.showing = true;
c2s.hiding = false;
c2s.img = loadGameImage(cndSrc + 'clicktostart.png'); // 337 x 75
...
sndo.div.style.backgroundImage = 'url("./images/soundoff.png")';
sndo.has_sound = true;
...
score.update = function() {
    for (var i = score.divs.length - 1; i >= 0; i--) {
		score.divs[i].style.top = 10;
		score.divs[i].style.fontSize = '40px';
...
score.endUpdate = function() {
    ...
    for (var i = score.divs.length - 1; i >= 0; i--) {
		score.divs[i].style.fontSize = '30px';
		score.divs[i].style.top = Number(Number(gameover.img.style.top.split("px")[0]) + 150 )+ "px";
		score.divs[i].innerHTML = "得分：" + points + " | 最高：<span style='color:#F00;'>" + highscore + "</span>";
		var wxTitle = "比2048和Flappy Bird还要虐心的游戏我已经玩到"+points+"分啦！速度来挑战我！";
		setWxContent(wxTitle);
		var highValue = new Object();
		if(highscore == 0){
			highscore = 1;
		}
		highValue["gameid"] = 2;
		highValue["key"] = gameLocal.getUserInfoKey();
		highValue["score"] = highscore;
		//alert("rrr - " + rank[0]);
		//alert(points);
		//dp_submitScore(-1,points);
        var str = points;
...
setWxContent = function (title) {
	if(typeof(window.shareData) != "undefined" ){
		...
		window.shareData["wContent"] = title + " —— " + window.shareData["fContent"] + "【http://www.91jiao.com/flappy2048/】";
	}
